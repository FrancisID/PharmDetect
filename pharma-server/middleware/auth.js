...
// Attaches req.user (the full account row) on success.
  ...
  return (req, res, next) => {
    ...
    try {
      ...
      if (user.status !== 'approved') {
        return res.status(403).json({ error: `Your account is ${user.status} — an administrator must approve it before you can access the Pharmaceutical Database.` });
      }
